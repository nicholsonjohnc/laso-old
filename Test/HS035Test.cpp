...
namespace LASO
{
	...
	public:
		...
		TEST_METHOD(fObjEvaluation)
		{
			...
			integer n = 3;
			LASO::HS035 hs035;
			vector x = { 0.5, 0.5, 0.5 };
			vector lb = { 0.0, 0.0, 0.0, 0.0 };
			...
			vector fCon(m);
			vector gCon(m * n);
			LASO::ProblemAL problem(n, hs035, &x[0], lb, ub, m, options, &fObj[0], &gObj[0], &fCon[0], &gCon[0]);
			problem.functions.objFun(problem.xDesignVariables_k, problem.fObjectiveFunction_k);
			Assert::AreEqual(2.25, problem.fObjectiveFunction_k[0]);
		...
		TEST_METHOD(gObjEvaluation)
		{
			...
			Assert::AreEqual(4.0, problem.objectiveGradient_k[1]);
			Assert::AreEqual(4.0, problem.objectiveGradient_k[2]);
		...
		TEST_METHOD(fConEvaluation)
		{
			...
			Assert::AreEqual(-6.0, problem.fConstraintFunctions_k[0]);
		...
		TEST_METHOD(jConEvaluation)
		{
			...
			Assert::AreEqual(-2.0, problem.jConstraintJacobian_k[2 * m + 0]);
		...
		TEST_METHOD(HS035)
		{
			...
			vector x = { 0.5, 0.5, 0.5 };
			vector lb = { 0.0, 0.0, 0.0, 0.0 };
			vector ub = { INFINITY, INFINITY, INFINITY, INFINITY };
			integer m = 1;
			LASO::Options options;
			vector fObj(1);
			vector gObj(n);
