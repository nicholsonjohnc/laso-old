...
namespace LASO
{
	...
	public:
		...
		TEST_METHOD(fObjEvaluation)
		{
			...
			vector fCon(m);
			vector gCon(m * n);
			LASO::ProblemAL problem(n, hs022, &x[0], lb, ub, m, options, &fObj[0], &gObj[0], &fCon[0], &gCon[0]);
		...
		TEST_METHOD(fConEvaluation)
		{
			...
			Assert::AreEqual(-2.0, problem.fConstraintFunctions_k[0]);
		...
		TEST_METHOD(HS022)
		{
			...
			problem.pgPenalties_k[0] = 100.0;
			problem.pgPenalties_k[1] = 100.0;
