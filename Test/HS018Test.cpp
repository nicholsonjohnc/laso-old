...
namespace LASO
{
	...
	public:
		...
		TEST_METHOD(fObjEvaluation)
		{
			...
			LASO::HS018 hs018;
			vector x = { 1.0, 2.0 };
			vector lb = { 2.0, 0.0, 0.0, 0.0 };
			vector ub = { 50.0, 50.0, INFINITY, INFINITY };
			integer m = 2;
			...
			vector fCon(m);
			vector gCon(m * n);
			LASO::ProblemAL problem(n, hs018, &x[0], lb, ub, m, options, &fObj[0], &gObj[0], &fCon[0], &gCon[0]);
			problem.functions.objFun(problem.xDesignVariables_k, problem.fObjectiveFunction_k);
			Assert::AreEqual(4.0099999999999998, problem.fObjectiveFunction_k[0]);
		...
		TEST_METHOD(fConEvaluation)
		{
			...
			Assert::AreEqual(-23.0, problem.fConstraintFunctions_k[0]);
			Assert::AreEqual(-20.0, problem.fConstraintFunctions_k[1]);
